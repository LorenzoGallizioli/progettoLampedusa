...
$\text{Formula1} = \frac{\text{Rimbalzi offensivi in attacco}}{\text{Tiri sbagliati su azione}}$ Rappresenta la capacità della squadra di ottenere un rimbalzo in attacco.

$\text{Formula2} = \frac{\text{Rimbalzi difensivi in difesa presi}}{\text{Tiri sbagliati su azione degli avversari}}$ Rappresenta la capacità della squadra di ottenere un rimbalzo in difesa, che troviamo un buon stimatore della capacità di contropiede della squadra.

$\text{Formula3} = \frac{\text{Rimbalzi ottenuti in attacco} + 1.5 \times \text{Rimbalzi ottenuti in difesa}}{\text{Rimbalzi subiti in attacco} + 2 \times \text{Rimbalzi subiti in difesa}}$ Rappresenta il rapporto tra i rimbalzi ottenuti (sia offensivi che difensivi) rispetto ai rimbalzi subiti (sia offensivi che difensivi). I coefficienti sono stati scelti in base a ciò che riteniamo più importante in una partita, ossia la difesa del proprio canestro.

$\text{Formula4} = (\text{Rimbalzi ottenuti in attacco - Rimbalzi subiti in attacco}) + 1.5*(\text{Rimbalzi ottenuti in difesa - Rimbalzi subiti in difesa})$ Cresce all'aumentare dei rimbalzi ottenuti e diminuisce all'aumentare dei rimbalzi subiti, considerando anche un coefficiente che da particolare importanza alla difesa.
...
Coefficiente che diminuisce esponenzialmente all'aumentare dei rimbalzi subiti

$\text{Formula7} = \text{Formula1} + \text{Formula2}$
Somma delle delle due formule capacità di ottenere un rimbalzo di attacco e in difesa.

$\text{Formula8} = \frac{\text{Rimbalzi ottenuti in attacco}}{\text{Totale rimbalzi ottenuti}}\times100$
Percentuale di rimbalzi ottenuti in attacco sui rimbalzi ottenuti.
...
df = subset(df, select = c("o_oreb", "o_dreb", "o_reb", "d_oreb", "o_fga", "o_fgm", "o_to", "d_dreb", "d_reb","d_fga", "d_fgm", "d_to", "won", "tmID"))
...
df$f7 <- (df$f1 + df$f2)^2
df$f8 <- (df$o_oreb)/(df$o_reb)*100

linMod <- lm(won ~ f1 + f2 + f3 + f4 + f5 + f6 + f7 + f8, data = df)
summary (linMod)
...
sample <- sample(c(TRUE, FALSE), nrow(df_1), replace=TRUE, prob=c(0.7, 0.3))

df_1 = subset(df_1, select = c("f1_z", "f2_z", "f3_z", "f4_z", "f5_z", "won"))
...
x <- data.matrix(train[, c("f1_z", "f2_z", "f3_z", "f4_z", "f5_z")])
...
#Lasso
new = data.matrix(test[,c("f1_z","f2_z","f3_z","f4_z","f5_z")])
prevLasso = predict(best_model, s = best_lambda, newx = new)

#LM
new = subset(test, select = c("f1_z","f2_z","f3_z","f4_z","f5_z"))
