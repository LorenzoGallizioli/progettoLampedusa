...
# Anderson-Darling Test per la normalità dei dati nella variabile 'reb' (rimbalzi)
ad.test(df$reb)
```
...
```{r}
# Questo codice esegue il test di Kolmogorov-Smirnov per confrontare la distribuzione della variabile 'reb' nel tuo dataset (df) con una distribuzione normale.
...
#TEST SHAPIRO WILK
```{r}
# Questo codice esegue il test di Shapiro-Wilk sulla variabile 'reb' nel tuo dataset (df)
...
#ATTENZIONE: facendo il train sul valore delle variabili, questo significa che sono esse ad essere i nostri dati, non i rimbalzi in se. Importante non confondersi durante l'esposizione.
...
plot(linMod, which = 1, col = "skyblue", pch = 16, main = "Residui Standardizzati")
abline(h = 0, col = "red", lty = 2)  # Aggiungi una linea orizzontale a zero nel grafico dei residui standardizzati
# Grafico dei livelli
plot(linMod, which = 2, col = "lightgreen", pch = 16, main = "Grafico dei Livelli")
# Grafico della distribuzione dei residui
plot(linMod, which = 3, col = "pink", pch = 16, main = "Distribuzione dei Residui")
# Grafico di Q-Q plot dei residui
plot(linMod, which = 4, col = "orange", pch = 16, main = "Q-Q Plot dei Residui")
...
# Crea il nuovo modello lineare normalizzato
...
# Supponendo che tu stia utilizzando la funzione influencePlot
influencePlot(linModNormalized, id = 5)

# Personalizzazione del grafico
par(mar=c(5, 5, 2, 2))  # Regola i margini per evitare sovrapposizioni con il titolo
title(main = "Influence Plot", col.main = "blue", font.main = 4)  # Aggiunge un titolo personalizzato

# Apri una nuova finestra grafica per il secondo grafico
dev.new()

# Supponendo che tu stia utilizzando la funzione influencePlot per il secondo grafico
...
# Stampa i coefficienti del modello LASSO e il summary del modello lineare normalizzato precedente per confronto.
...
# Questo codice esegue un test ANOVA per le variabili categoriche 'confID' e 'divID', creando variabili dummy e aggiungendole al dataframe. Successivamente, viene eseguito un modello lineare    con la variabile normalizzata e 'divID' come predittore.
...
# Si tiene l'ipotesi nulla perché la variabile 'conference' non ha un effetto sulle vittorie.
# Non posso inserire la variabile nel modello.

# VARIABILI DUMMY CON TEST ANOVA su DivisionID

# Controlla i valori univoci nella colonna 'division'
unique_divisions <- unique(df$divID)
print(unique_divisions)

# Supponiamo che 'division' sia la variabile categorica di interesse
df$division <- as.factor(df$divID)

# Creazione di variabili dummy
dummy_vars_division <- model.matrix(~ df$divID - 1, data = df)  # -1 per evitare la colinearità

# Aggiunta delle variabili dummy al dataframe
df <- cbind(df, dummy_vars_division)

# Esecuzione di un test ANOVA
# Supponiamo che 'won' sia la variabile dipendente
anova(lm(df$won ~ df$divID, data = df))

# Si rifiuta l'ipotesi nulla perché la variabile 'division' ha un effetto sulle vittorie.
# Posso inserire la variabile nel modello.

# Creazione del modello lineare con variabili normalizzate e 'division' come predittore
...
# Questo codice aggiorna il modello lineare (linModNormalized1) togliendo la seconda interazione non significativa tra 'f4_z' e 'divID'. Il nuovo modello è rappresentato da linModNormalized2,   che comprende le variabili normalizzate, la variabile categorica 'divID', e l'interazione significativa tra 'f1_z' e 'f2_z'.

# Creazione del modello lineare con variabili normalizzate, 'divID', e interazione significativa tra 'f1_z' e 'f2_z'
linModNormalized1 <- lm(won ~ f1_z + f2_z + f3_z + f4_z + f5_z + divID + f1_z:f2_z + f4_z:divID, data = df)
summary(linModNormalized1)

# f4 è un indicatore che cresce all'aumentare dei rimbalzi ottenuti e diminuisce all'aumentare dei rimbalzi subiti.
# L'interazione tra f4 e la divisione non è significativa, mentre l'interazione tra f1 e f2 è significativa.

# AGGIORNAMENTO del modello lineare togliendo la seconda interazione non significativa
...
# Riassunto del modello lineare aggiornato
summary(linModNormalized2)
...
# Creazione di un modello di regressione generalizzata di Poisson
linModNormalized2_pois <- glm(won ~ f1_z + f2_z + f3_z + f4_z + f5_z + divID + f1_z:f2_z, 
                              family = poisson(link = log), data = df)
summary(linModNormalized2_pois)

# Ottenimento dei coefficienti per entrambi i modelli
