...
- [Confronto dei Modelli Prima e Dopo la Rimozione degli Outliers](#confronto-dei-modelli-prima-e-dopo-la-rimozione-degli-outliers)
   * [Prima della Rimozione degli Outliers:](#prima-della-rimozione-degli-outliers)
   * [Dopo la Rimozione degli Outliers:](#dopo-la-rimozione-degli-outliers)
   * [Grafico di Influenza:](#grafico-di-influenza)
      + [Visualizzazione Residui e Leverage Prima e Dopo la Rimozione degli Outliers](#visualizzazione-residui-e-leverage-prima-e-dopo-la-rimozione-degli-outliers)
- [Implementazione del Modello LASSO per l'Ottimizzazione della Regressione](#implementazione-del-modello-lasso-per-lottimizzazione-della-regressione)
   * [1. Preparazione e Valutazione del Modello di Regressione LASSO](#1-preparazione-e-valutazione-del-modello-di-regressione-lasso)
      + [Allenamento modello LASSO](#allenamento-modello-lasso)
   * [2. Valutazione Comparativa delle Previsioni: LASSO contro Modello Lineare](#2-valutazione-comparativa-delle-previsioni-lasso-contro-modello-lineare)
   * [3. Valutazione delle Prestazioni del Modello](#3-valutazione-delle-prestazioni-del-modello)
...
- [Analisi della Varianza (ANOVA) per Valutazione di Fattori Categorici](#analisi-della-varianza-anova-per-valutazione-di-fattori-categorici)
   ...
   * [Esame dell'Effetto della Variabile 'divID'](#esame-delleffetto-della-variabile-divid)
...
- [Valutazione dei modelli mediante l'Akaike Information Criterion](#valutazione-dei-modelli-mediante-lakaike-information-criterion)
- [Conclusioni e risultati](#conclusioni-e-risultati)
...
cat(paste("Percentuale di valori non-zero in o_oreb: ", round(length(which(df_reb$o_oreb > 0)) / dim(df_reb)[1] * 100, 2), "%\n",
          "Percentuale di valori non-zero in o_dreb: ", round(length(which(df_reb$o_dreb > 0)) / dim(df_reb)[1] * 100, 2), "%\n",
          "Percentuale di valori non-zero in o_reb: ", round(length(which(df_reb$o_reb > 0)) / dim(df_reb)[1] * 100, 2), "%\n",
          "Percentuale di valori non-zero in d_oreb: ", round(length(which(df_reb$d_oreb > 0)) / dim(df_reb)[1] * 100, 2), "%\n",
          "Percentuale di valori non-zero in d_dreb: ", round(length(which(df_reb$d_dreb > 0)) / dim(df_reb)[1] * 100, 2), "%\n",
          "Percentuale di valori non-zero in d_reb: ", round(length(which(df_reb$d_reb > 0)) / dim(df_reb)[1] * 100, 2), "%", sep = ""))
...
Nel seguente frammento di codice abbiamo eseguito la normalizzazione delle variabili all'interno dei nostri dataframe 'train' e 'test', facendo ricorso alla funzione `scale`. Ciò garantisce che ogni variabile apporti un contributo proporzionato all'interno del modello, consentendo un'interpretazione più precisa e accurata dei coefficienti derivanti dalla regressione lineare.
...
### Visualizzazione Residui e Leverage Prima e Dopo la Rimozione degli Outliers
...
- Grafico prima della rimozione outliers
...
- Grafico dopo la rimozione outliers
...
- Calcolo del RMSE (Root Mean Square Error)  
Questi blocchi di codice calcolano il RMSE, una misura dell'errore di previsione, per il modello LASSO e il modello lineare (LM). Il RMSE fornisce una stima dell'errore quadratico medio tra i valori previsti e i valori reali.
```{r}
...
cat(paste("Accuratezza: ", round(accuracy * 100, digits = 4), "%\n",
          "Precisione: ", round(precision * 100, digits = 4), "%\n",
          "Sensibilità: ", round(sensitivity * 100, digits = 4), "%\n",
          "F-Score: ", round(fscore * 100, digits = 4), "%\n",
          "Specificità: ", round(specificity * 100, digits = 4), "%\n",
          "RMS Lasso: ", round(rmsLasso, digits = 4), "\n",
          "RMS Modello Lineare: ", round(rmsLM, digits = 4), sep = ""))
...
cat("AIC per il modello lineare (linModNormalized_2): ", round(aic_2, digits = 4), "\n",
    "AIC per il modello di Poisson (linModNormalized_2_pois): ", round(aic_2_pois, digits = 4), "\n", sep = "")
...
1. **Importanza dei Rimbalzi nel Basket**: Il modello lineare ha dimostrato che esiste una correlazione positiva tra il numero di rimbalzi e le vittorie delle squadre. Ciò suggerisce che le squadre che eccellono nel rimbalzo tendono a ottenere migliori prestazioni in termini di vittorie. Questo risultato sottolinea l'importanza strategica dei rimbalzi nel gioco del basket, incoraggiando le squadre a focalizzarsi su questa area durante gli allenamenti e le strategie di gioco.

2. **Effetti dei Rimbalzi Offensivi e Difensivi**: Analizzando separatamente i rimbalzi offensivi e difensivi, il modello ha rivelato che entrambi contribuiscono significativamente alle prestazioni della squadra, sebbene in misura diversa. I rimbalzi difensivi sembrano avere un impatto leggermente maggiore sulle vittorie rispetto a quelli offensivi. Questo può guidare gli allenatori a enfatizzare la difesa del canestro e il controllo del tabellone difensivo come elementi chiave per il successo della squadra.

3. **Strategie di Gioco e Formazione della Squadra**: Questi risultati possono influenzare le decisioni relative alla formazione della squadra e alle strategie di gioco. Ad esempio, squadre con un basso numero di rimbalzi potrebbero considerare l'introduzione di giocatori più forti nel rimbalzo o modificare le loro tattiche di gioco per migliorare in questa area. Inoltre, l'allenamento potrebbe essere più focalizzato su esercizi che migliorano la capacità di rimbalzo e la posizione sotto il canestro.
...
# Bibliografia
1. "Introduzione a R" - Claudio Agostinelli (https://cran.r-project.org/doc/contrib/manuale.0.3.pdf)
2. "An Introduction to R" - W. N. Venables, D. M. Smith and the R Core Team (https://cran.rproject.org/doc/manuals/r-release/R-intro.pdf)
3. "R Essentials" (https://bookdown.org/content/4e34e34f-ca48-4090-90ca-8ae7b1b65e0e/plotting-with-r-base-code.html)
4. "Metulini_Gnecco_2022_ANOR-2.pdf" (https://elearning15.unibg.it/pluginfile.php/441960/mod_folder/content/0/Metulini_Gnecco_2022_ANOR.pdf?forcedownload=1)
5. Un grazie ad Anna Giabelli per il materiale che ci ha offerto
